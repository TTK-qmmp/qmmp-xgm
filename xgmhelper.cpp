...
#include <libxgm/klystrack/lib/ksnd.h>
...
{
    return m_path.contains("://") ? AbstractReader::pathFromUrl(m_path) : m_path;
}

QString AbstractReader::pathFromUrl(const QString &url, int *track)
{
    if(track)
    {
        *track = -1;
...
//        qWarning("KlystrackReader: KSND_CreatePlayerUnregistered failed, %s", qPrintable(path));
    ...
    m_song = KSND_LoadSong(m_player, qPrintable(path));
    if(!m_song)
    {
